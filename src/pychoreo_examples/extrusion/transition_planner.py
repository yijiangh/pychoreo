...
def solve_transition_between_extrusion_processes(robot, ik_joints, print_trajs, element_bodies, initial_conf,
    ...
    for seq_id in range(len(print_trajs)+1):
        if seq_id < len(print_trajs):
            ...
            print_traj = print_trajs[seq_id]
            if seq_id != 0:
                tr_start_conf = print_trajs[seq_id-1].traj_path[-1]
            else:
                tr_start_conf = initial_conf
            tr_end_conf = print_trajs[seq_id].traj_path[0]
        elif return2idle:
            print('plan for returning to idle position.')
            tr_start_conf = print_trajs[-1].traj_path[-1]
        ...
        if seq_id < len(print_trajs):
            built_obstacles.append(element_bodies[print_traj.element])
    return trans_traj
